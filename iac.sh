...
IAC_PAYLOAD=$(printf "$IAC_PAYLOAD_PRE" "$IAC_ASSET_NAME" "$IAC_ASSET_TYPE" "$TAG_KEY_1" "$TAG_VALUE_1" "$TAG_KEY_2" "$TAG_VALUE_2" "$TAG_KEY_3" "$TAG_VALUE_3" "$SCAN_ATTRIBUTE_KEY" "$SCAN_ATTRIBUTE_VALUE" "$SCRIPT_KEY" "$SCRIPT_VALUE" "$FAILURE_CRITERIA_HIGH" "$FAILURE_CRITERIA_MED" "$FAILURE_CRITERIA_LOW" "$FAILURE_OPERATOR")
...
IAC_VAR_FILE=""
IAC_POLICY_ID=""
IAC_FILE_TO_SCAN="/home/prisma-presenter/Project/terragoat.zip"
IAC_FOLDER_TO_SCAN=""
...
IAC_CONFIG_PRE='{
  "data": {
    ...
    "attributes": {
      ...
      "templateParameters": {
        "variableFiles": [
          "./terragoat/terraform/aws/consts.tf",
	  "./terragoat/terraform/azure/variables.tf",
	  "./terragoat/terraform/gcp/variables.tf"
        ]
      }
    }
  }
...
# Full config template for customizing
...
# Calls the full config template needed if customizing

# IAC_CONFIG=$(printf "$IAC_CONFIG_PRE" "$PC_SCAN_ID" "$IAC_TEMPLATE_TYPE" "$IAC_TEMPLATE_VERSION" "$IAC_VAR_PROPERTY1" "$IAC_VAR_VALUE1" "$IAC_VAR_PROPERTY2" "$IAC_VAR_VALUE2" "$IAC_VAR_FILE" "$IAC_POLICY_ID" "$IAC_FILE_TO_SCAN" "$IAC_FOLDER_TO_SCAN")

IAC_CONFIG=$(printf "$IAC_CONFIG_PRE" "$PC_SCAN_ID" "$IAC_TEMPLATE_TYPE" "$IAC_TEMPLATE_VERSION")
...
IAC_RESULTS=$(curl -s \
                   --request GET \
                   --url "${PC_APIURL}/iac/v2/scans/${PC_SCAN_ID}/results" \
                   --header "Content-Type: application/json" \
                   --header "x-redlock-auth: ${PC_JWT}")
